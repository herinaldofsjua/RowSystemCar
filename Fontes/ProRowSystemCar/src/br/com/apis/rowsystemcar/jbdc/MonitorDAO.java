...
import br.com.apis.rowsystemcar.entidades.Monitor;
import br.com.apis.rowsystemcar.entidades.Monitor;
...
public class MonitorDAO {
	...
	private void alterar(Monitor monitor) {
		...
		String consultasql = "UPDATE monitor SET nome=?,login=?, senha=?, telefone1,cidade bairro=?, rua=?, numero=?, uf=?  WHERE ?";
		try {
			PreparedStatement preparador = conexao.prepareStatement(consultasql);// cria
			preparador.setString(1, monitor.getNome());
			preparador.setString(2, monitor.getLogin());
	...
	public void excluir(Monitor monitor) {
		String consultasql = "DELETE FROM monitor WHERE id=?";
		try {
			...
			JOptionPane.showMessageDialog(null, "Monitor não existe!" + e.getMessage());
		}
	}
